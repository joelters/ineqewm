...
#' @param D Treatment indicator, used to compute the fraction of
#' inidividuals actually treated in the data within the groups
#' generated by the treatment rule
#' @param ate_scores the AIPW scores used to estimate the ATE. Used to compute
#' the CATE in the groups created by the treatment rules.
...
#' @param targetXnames_pretty string vector with the names we wish to see in the
#' output tree (e.g Mother education instead of educm).
#' @param roundvalues whether we want the values of the cutting points in the tree to
#' appear rounded in the tree
#' @param roundcate To how many decimals do we wish to round the CATE of the groups
#' generated by the treatment rule.
#' @return A list with the output and a figure.
#' @export
ineqewm8 <-function(scores11 = NULL,
                    scores10 = NULL,
                    scores01 = NULL,
                    scores00 = NULL,
                    D = NULL,
  ...
  if (rule == "tree"){
    ...
    if (res[1,8] == 0){
      return(list(Welfare = data.frame(Welfare = res[1,7],
                              Welfare.nontreated = res[1,9],
                              Welfare.alltreated = res[1,10]),
                  optimal.rule = res[,base::ifelse(welfare == "igm",13,12)],
                  tree = "Treat no one"))
    }
    ...
    # Optimal tree has depth 1
    else if (res[1,8] == 9912 | res[1,8] == 9921){
      if (is.null(targetXnames_pretty) & roundvalues == FALSE){
        res2 <- list(Welfare = res[1,7],
                     node0 = colnames(targetX[,res[1,1]]),
                     thres0 = s[res[1,2],res[1,1]],
                     rule = data.table::fifelse(res[1,8] == 9912,"rl_tn","rl_nt"))
      }
      else if (is.null(targetXnames_pretty) & roundvalues == TRUE){
        res2 <- list(Welfare = res[1,7],
                     node0 = colnames(targetX[,res[1,1]]),
                     thres0 = round(s[res[1,2],res[1,1]]),
                     rule = data.table::fifelse(res[1,8] == 9912,"rl_tn","rl_nt"))
      }
      else if (!is.null(targetXnames_pretty) & roundvalues == FALSE){
        res2 <- list(Welfare = res[1,7],
                     node0 = targetXnames_pretty[res[1,1]],
                     thres0 = s[res[1,2],res[1,1]],
                     rule = data.table::fifelse(res[1,8] == 9912,"rl_tn","rl_nt"))
      }
      else if (!is.null(targetXnames_pretty) & roundvalues == TRUE){
      ...
      tn1 <- paste(data.table::fifelse(substr(res2$rule,4,4) == "t",
                                       "Treat \n n = ","Do not treat \n n = "),
                   sum(targetX[,colnames(targetX[,res[1,1]])] <= s[res[1,2],res[1,1]]),
                   "\n CATE = ",
                   round(mean(ate_scores[targetX[,colnames(targetX[,res[1,1]])] <= s[res[1,2],res[1,1]]]),roundcate),
                   "\n E(D) = ",
                   round(mean(D[targetX[,colnames(targetX[,res[1,1]])] <= s[res[1,2],res[1,1]]]),2))
      tn2 <- paste(data.table::fifelse(substr(res2$rule,5,5) == "t",
                                       "Treat \n n = ","Do not treat \n n = "),
                   sum(targetX[,colnames(targetX[,res[1,1]])] > s[res[1,2],res[1,1]]),
                   "\n CATE = ",
                   round(mean(ate_scores[targetX[,colnames(targetX[,res[1,1]])] > s[res[1,2],res[1,1]]]),roundcate),
      ...
      WT0 <- res[1,9]
      WT1 <- res[1,10]
      perctreated <- mean(res[,12])
      if (welfare == "igm"){
        return(list(Welfare = data.frame(Welfare = res2$Welfare,
                                         tau = res[2,1],
                                         Welfare.nontreated = WT0,
                                         tau.nontreated = res[2,2],
                                         Welfare.alltreated = WT1,
                                         tau.alltreated = res[2,3],
                                         share.treated = perctreated),
                    optimal.rule = res[,12],
                    tree = tree,
                    tnode = res[,11]))
      }
    ...
    # optimal tree has depth 2
      ...
      if (is.null(targetXnames_pretty) & roundvalues == FALSE){
        ...
        res2 <- list(Welfare = res[1,7],
                     node0 = targetXnames_pretty[res[1,1]],
                     thres0 = s[res[1,2],res[1,1]],
                     node1 = targetXnames_pretty[res[1,3]],
                     thres1 = s[res[1,4],res[1,3]],
                     node2 = targetXnames_pretty[res[1,5]],
                     thres2 = s[res[1,6],res[1,5]],
                     rule = rls[res[1,8]])
      }
      else if (is.null(targetXnames_pretty) & roundvalues == TRUE){
        res2 <- list(Welfare = res[1,7],
                     ...
                     thres0 = round(s[res[1,2],res[1,1]]),
                     node1 = colnames(targetX[,res[1,3]]),
                     thres1 = round(s[res[1,4],res[1,3]]),
                     node2 = colnames(targetX[,res[1,5]]),
                     thres2 = round(s[res[1,6],res[1,5]]),
                     rule = rls[res[1,8]])
      }
      else if (!is.null(targetXnames_pretty) & roundvalues == TRUE){
        res2 <- list(Welfare = res[1,7],
      ...
      tn1 <- paste(data.table::fifelse(substr(res2$rule,4,4) == "t",
                   "Treat \n n = ","Do not treat \n n = "),
                   sum(targetX[,colnames(targetX[,res[1,1]])] <= s[res[1,2],res[1,1]] &
                       targetX[,colnames(targetX[,res[1,3]])] <= s[res[1,4],res[1,3]]),
                   "\n CATE = ",
                   round(mean(ate_scores[targetX[,colnames(targetX[,res[1,1]])] <= s[res[1,2],res[1,1]] &
                                         targetX[,colnames(targetX[,res[1,3]])] <= s[res[1,4],res[1,3]]]),roundcate),
                   "\n E(D) = ",
                   round(mean(D[targetX[,colnames(targetX[,res[1,1]])] <= s[res[1,2],res[1,1]] &
                                targetX[,colnames(targetX[,res[1,3]])] <= s[res[1,4],res[1,3]]]),2))
      tn2 <- paste(data.table::fifelse(substr(res2$rule,5,5) == "t",
                   "Treat \n n = ","Do not treat \n n = "),
                   sum(targetX[,colnames(targetX[,res[1,1]])] <= s[res[1,2],res[1,1]] &
                       targetX[,colnames(targetX[,res[1,3]])] > s[res[1,4],res[1,3]]),
                   ...
                   round(mean(D[targetX[,colnames(targetX[,res[1,1]])] <= s[res[1,2],res[1,1]] &
                                targetX[,colnames(targetX[,res[1,3]])] > s[res[1,4],res[1,3]]]),2))
      tn3 <- paste(data.table::fifelse(substr(res2$rule,6,6) == "t",
                   "Treat \n n = ","Do not treat \n n = "),
                   sum(targetX[,colnames(targetX[,res[1,1]])] > s[res[1,2],res[1,1]] &
                       targetX[,colnames(targetX[,res[1,5]])] <= s[res[1,6],res[1,5]]),
                   "\n CATE = ",
                   round(mean(ate_scores[targetX[,colnames(targetX[,res[1,1]])] > s[res[1,2],res[1,1]] &
                                         targetX[,colnames(targetX[,res[1,5]])] <= s[res[1,6],res[1,5]]]),roundcate),
                   "\n E(D) = ",
                   round(mean(D[targetX[,colnames(targetX[,res[1,1]])] > s[res[1,2],res[1,1]] &
                                targetX[,colnames(targetX[,res[1,5]])] <= s[res[1,6],res[1,5]]]),2))
      tn4 <- paste(data.table::fifelse(substr(res2$rule,7,7) == "t",
                   "Treat \n n = ","Do not treat \n n = "),
                   sum(targetX[,colnames(targetX[,res[1,1]])] > s[res[1,2],res[1,1]] &
                       targetX[,colnames(targetX[,res[1,5]])] > s[res[1,6],res[1,5]]),
                   "\n CATE = ",
